...
export class Summarize {
  ...
  async run(opts: SummarizeOptions): Promise<SummarizeResult> {
    ...
    // Cross-scope save guard: an unscoped retrieval pulls hits from EVERY
    // readable scope, and `save: true` would then persist the
    // LLM-synthesized result into the `default` scope via Memories.remember.
    // That silently smuggles content from tighter scopes into the
    // broadly-readable default. Require scope at the API boundary so no
    // caller (CLI, MCP, programmatic) can land in that pattern by accident.
    if (opts.save && !opts.scope) {
      throw new ValidationError(
        'scope',
        '--scope is required with --save: an unscoped summary would mix readable scopes and persist them into `default`'
      );
    }
